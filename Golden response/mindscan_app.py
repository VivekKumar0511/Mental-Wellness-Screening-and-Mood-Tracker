"""
MindScan — Single File Version (no frontend.py needed)
Full-Stack Mental Wellness Screening & Mood Tracker
"""
...
JWT_SECRET  = os.getenv("JWT_SECRET", "mindscan_dev_secret")
MONGO_URI   = os.getenv("MONGO_URI",  "mongodb://localhost:27017/mindscan")
PORT        = int(os.getenv("PORT",   5000))
MODEL_PATH  = "ml_models/mindscan_model.pkl"
...
DASS_DEPRESSION_IDX = [2, 4, 9, 12, 15, 16, 20]
DASS_ANXIETY_IDX    = [1, 3, 6, 8, 14, 18, 19]
DASS_STRESS_IDX     = [0, 5, 7, 10, 11, 13, 17]


def classify_severity(score, thresholds):
    if score <= thresholds[0]: return "Minimal"
    elif score <= thresholds[1]: return "Mild"
    elif score <= thresholds[2]: return "Moderate"
    return "Severe"
...
def calculate_scores(answers):
    ...
    return {
        "phq9":       sum(answers[:9]),
        "depression": sum(answers[i] for i in DASS_DEPRESSION_IDX) * 2,
        "anxiety":    sum(answers[i] for i in DASS_ANXIETY_IDX)    * 2,
        "stress":     sum(answers[i] for i in DASS_STRESS_IDX)     * 2
    }


def compute_wellness_score(dep, anx, stress):
    normalised = (dep * 0.35 + anx * 0.30 + stress * 0.35) / 42
    return max(0, round(100 - (normalised * 100)))


def train_model():
    os.makedirs("ml_models", exist_ok=True)
    print("Training ML model on synthetic DASS-21 data...")
    np.random.seed(42)
    X_raw, y_raw = [], []
    for _ in range(5000):
        answers = np.random.randint(0, 4, 21).tolist()
        scores  = calculate_scores(answers)
        X_raw.append([scores["phq9"], scores["depression"], scores["anxiety"], scores["stress"]])
        y_raw.append([
            classify_severity(scores["depression"], [9, 13, 20]),
            classify_severity(scores["anxiety"],    [7, 9, 14]),
            classify_severity(scores["stress"],     [14, 18, 25])
        ])
    scaler   = StandardScaler()
    X_scaled = scaler.fit_transform(np.array(X_raw))
    model    = MultiOutputClassifier(RandomForestClassifier(n_estimators=100, random_state=42))
    model.fit(X_scaled, np.array(y_raw))
...
def signup():
    try:
        ...
        users_col.insert_one({"name": name, "email": email, "password": hashed, "createdAt": datetime.datetime.utcnow()})
...
def login():
    try:
        ...
        token = jwt.encode({"user_id": str(user["_id"]), "exp": datetime.datetime.utcnow() + datetime.timedelta(days=7)}, JWT_SECRET, algorithm="HS256")
...
def submit_screening():
    try:
        ...
        if len(answers) != 21:
            return jsonify({"message": f"Expected 21 answers, got {len(answers)}"}), 400
        for i, a in enumerate(answers):
            if a not in [0, 1, 2, 3]:
                return jsonify({"message": f"Answer at index {i} must be 0-3"}), 400
        scores   = calculate_scores(answers)
        features = np.array([[scores["phq9"], scores["depression"], scores["anxiety"], scores["stress"]]])
        scaled   = scaler.transform(features)
        prediction  = model.predict(scaled)[0]
        pred_labels = {"depression": prediction[0], "anxiety": prediction[1], "stress": prediction[2]}
        wellness    = compute_wellness_score(scores["depression"], scores["anxiety"], scores["stress"])
        screenings_col.insert_one({
            "userId": request.user_id, "answers": answers,
            "phq9Score": scores["phq9"],
            "dass21": {"depression": scores["depression"], "anxiety": scores["anxiety"], "stress": scores["stress"]},
            "prediction": pred_labels, "wellnessScore": wellness, "date": datetime.datetime.utcnow()
        })
        return jsonify({"wellnessScore": wellness, "prediction": pred_labels, "scores": scores, "message": "Screening completed"}), 200
    except Exception as e:
        return jsonify({"message": f"Server error: {str(e)}"}), 500
...
def screening_history():
    records = list(screenings_col.find({"userId": request.user_id}, {"_id": 0}).sort("date", -1))
    for r in records:
...
def add_journal():
    try:
        ...
        entry = {
            "userId": request.user_id, "mood": data.get("mood"),
            "moodScore": data.get("moodScore"), "entryText": data.get("entryText", "")[:2000],
            "tags": data.get("tags", []), "date": data.get("date", datetime.datetime.utcnow().isoformat()),
            "createdAt": datetime.datetime.utcnow()
        }
        result = journal_col.insert_one(entry)
        return jsonify({"message": "Journal entry saved", "id": str(result.inserted_id)}), 201
    except Exception as e:
        return jsonify({"message": f"Server error: {str(e)}"}), 500
...
@token_required
def get_journal():
    entries = list(journal_col.find({"userId": request.user_id}).sort("createdAt", -1))
    for e in entries:
        e["_id"] = str(e["_id"])
        if isinstance(e.get("createdAt"), datetime.datetime):
...
def update_journal(entry_id):
    try:
        data    = request.json or {}
        updated = journal_col.update_one(
            {"_id": ObjectId(entry_id), "userId": request.user_id},
            {"$set": {"entryText": data.get("entryText", "")[:2000], "tags": data.get("tags", []), "mood": data.get("mood"), "moodScore": data.get("moodScore")}}
        )
        if updated.matched_count == 0:
            return jsonify({"message": "Entry not found or access denied"}), 404
...
def delete_journal(entry_id):
    try:
        deleted = journal_col.delete_one({"_id": ObjectId(entry_id), "userId": request.user_id})
...
def get_resources():
    return jsonify({
        "meditation": [
            {"title": "Headspace — Guided Meditation", "link": "https://www.headspace.com"},
            {"title": "4-7-8 Breathing Exercise Guide", "link": "https://www.healthline.com/health/4-7-8-breathing"},
        ],
        "therapy": [
            {"title": "BetterHelp — Online Therapy", "link": "https://www.betterhelp.com"},
            {"title": "MindShift CBT App", "link": "https://www.anxietycanada.com/resources/mindshift-cbt/"},
        ],
        "crisis": [
            {"title": "iCall India Helpline — 9152987821", "link": "https://icallhelpline.org"},
            {"title": "Find A Helpline (Global)", "link": "https://findahelpline.com"},
        ],
        "selfHelp": [
            {"title": "Journaling for Mental Health Guide", "link": "https://positivepsychology.com/journaling/"},
            {"title": "Sleep Hygiene Tips", "link": "https://www.sleepfoundation.org/sleep-hygiene"},
        ]
    }), 200

# ==========================================
# 9. FRONTEND — INLINE HTML (no frontend.py needed)
# ==========================================
...
<head>
  ...
  <style>
    :root { --primary: #4A90D9; --green: #5BAD8F; --bg: #F4F6FB; --card: #FFFFFF; --text: #2D2D2D; --muted: #7A7A8C; --border: #E0E4EF; }
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Poppins', sans-serif; background: var(--bg); color: var(--text); }
    nav { display: none; justify-content: space-between; align-items: center; padding: 1rem 2rem; background: var(--card); border-bottom: 1px solid var(--border); }
    nav .brand { font-weight: 600; color: var(--primary); font-size: 1.2rem; }
    nav .nav-links a { margin-left: 1.5rem; cursor: pointer; color: var(--muted); text-decoration: none; font-size: 0.9rem; font-weight: 500; transition: color 0.2s; }
    nav .nav-links a:hover { color: var(--primary); }
    .view { display: none; min-height: calc(100vh - 60px); padding: 2rem; }
    .view.active { display: block; }
    .card { background: var(--card); border-radius: 16px; padding: 2rem; box-shadow: 0 4px 20px rgba(0,0,0,0.06); margin-bottom: 1.5rem; }
    .card h2 { color: var(--primary); margin-bottom: 1rem; font-size: 1.4rem; }
    .container { max-width: 480px; margin: 3rem auto; }
    .input { width: 100%; padding: 12px 16px; margin: 8px 0 16px; border: 1.5px solid var(--border); border-radius: 10px; font-family: 'Poppins', sans-serif; font-size: 0.95rem; transition: border-color 0.2s; outline: none; }
    .input:focus { border-color: var(--primary); }
    .btn { width: 100%; padding: 13px; border: none; border-radius: 10px; background: var(--primary); color: white; font-family: 'Poppins', sans-serif; font-size: 0.95rem; font-weight: 500; cursor: pointer; transition: opacity 0.2s; }
    .btn:hover:not(:disabled) { opacity: 0.88; }
    .btn:disabled { background: #ccc; cursor: not-allowed; }
    .btn-outline { background: transparent; color: var(--primary); border: 1.5px solid var(--primary); margin-top: 0.75rem; }
    ...
    .option { padding: 14px 18px; border: 2px solid var(--border); border-radius: 10px; margin-bottom: 10px; cursor: pointer; transition: border-color 0.2s, background 0.2s; font-size: 0.95rem; }
    .option:hover { border-color: var(--primary); }
    .option.selected { border-color: var(--primary); background: rgba(74,144,217,0.08); font-weight: 500; }
    .wellness-score { text-align: center; padding: 1.5rem; background: linear-gradient(135deg, #4A90D9, #5BAD8F); border-radius: 16px; color: white; margin-bottom: 1.5rem; }
    ...
    .badge { display: inline-block; padding: 4px 14px; border-radius: 20px; font-size: 0.8rem; font-weight: 600; margin-bottom: 0.5rem; }
    ...
    .crisis-alert { background: #fde8e8; border: 1.5px solid #f87171; border-radius: 12px; padding: 1rem 1.5rem; margin-bottom: 1.5rem; color: #7f1d1d; }
    .dash-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 1rem; margin-bottom: 1.5rem; }
    .stat-card { background: var(--card); border-radius: 12px; padding: 1.2rem; border: 1px solid var(--border); text-align: center; }
    ...
    .toast { position: fixed; bottom: 24px; right: 24px; padding: 12px 20px; border-radius: 10px; background: #2D2D2D; color: white; font-size: 0.9rem; z-index: 9999; animation: slideUp 0.3s ease; max-width: 320px; }
...
<div id="screening-view" class="view">
  <div style="max-width:640px; margin:0 auto;">
    <div id="disclaimer-card" class="card">
      ...
      <p style="color:var(--muted); line-height:1.8; margin-bottom:1.5rem;">This tool uses the DASS-21 questionnaire to screen for patterns of depression, anxiety, and stress. It is not a clinical diagnosis.</p>
    ...
    <div id="results-card" class="card" style="display:none;">
      <div id="crisis-alert" class="crisis-alert" style="display:none;">
        If you are in crisis, please call iCall: <strong>9152987821</strong> or visit <a href="https://findahelpline.com" target="_blank">findahelpline.com</a>
      </div>
      ...
      <div class="disclaimer">This is a wellness screening tool, not a medical diagnosis. Always consult a licensed professional.</div>
    </div>
  </div>
</div>
...
<div id="journal-view" class="view">
  <div style="max-width:680px; margin:0 auto;">
    <div class="card">
      ...
      <textarea id="journal-text" class="input" rows="5" maxlength="2000" placeholder="Write how you're feeling today..." style="resize:vertical;"></textarea>
...
<script>
const API = "";
let isLogin = true, currentQ = 0, answers = [], lastResult = null;
let journalTags = [], selectedMood = null, selectedMoodScore = null;
let allEntries = [], chartInstance = null;
...
function init() {
  const token = localStorage.getItem("token");
  if (token) { document.getElementById("navbar").style.display = "flex"; showView("dashboard-view"); }
...
async function loadDashboard() {
  ...
  try {
    const [screenings, entries] = await Promise.all([api("/api/screening/history"), api("/api/journal/entries")]);
    if (!screenings || !entries) return;
    document.getElementById("dash-wellness").innerText = screenings.length ? screenings[0].wellnessScore : "N/A";
    ...
    const moodMap = {};
    entries.forEach(e => { const d = e.date ? e.date.slice(0, 10) : ""; if (d) moodMap[d] = e.moodScore || 3; });
    const labels = [], dataPoints = [];
    for (let i = 29; i >= 0; i--) {
      const d = new Date(); d.setDate(d.getDate() - i);
      const key = d.toISOString().slice(0, 10);
      labels.push(key.slice(5)); dataPoints.push(moodMap[key] || null);
    }
    if (chartInstance) chartInstance.destroy();
    chartInstance = new Chart(document.getElementById("moodChart"), {
      type: "line",
      data: { labels, datasets: [{ label: "Mood Score", data: dataPoints, borderColor: "#4A90D9", backgroundColor: "rgba(74,144,217,0.08)", tension: 0.3, fill: true, pointRadius: 3, spanGaps: true }] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { min: 1, max: 5, ticks: { stepSize: 1 } }, x: { ticks: { autoSkip: true, maxTicksLimit: 10 } } } }
    });
...
function startScreening() {
  answers = new Array(21).fill(null); currentQ = 0; lastResult = null;
  document.getElementById("disclaimer-card").style.display = "none";
  document.getElementById("quiz-card").style.display = "block";
  renderQuestion();
}

function restartScreening() { resetScreeningUI(); startScreening(); }
...
function renderQuestion() {
  ...
  document.getElementById("q-content").innerHTML =
    ...
    opts.map((o, i) => "<div class='option " + (answers[currentQ] === i ? "selected" : "") + "' onclick='pickAnswer(" + i + ")'>" + o + "</div>").join("");
...
function pickAnswer(val) { answers[currentQ] = val; renderQuestion(); }
function prevQuestion() { if (currentQ > 0) { currentQ--; renderQuestion(); } }

async function nextQuestion() {
  if (currentQ < QUESTIONS.length - 1) { currentQ++; renderQuestion(); return; }
  const nextBtn = document.getElementById("next-btn");
  nextBtn.innerText = "Processing..."; nextBtn.disabled = true;
  try {
    const res = await api("/api/screening/submit", "POST", { answers });
    if (!res) return;
    lastResult = res; showResults(res);
  } catch(e) { toast(e.message, "error"); nextBtn.innerText = "Submit"; nextBtn.disabled = false; }
}
...
function showResults(res) {
  ...
  const desc = { Minimal: "You're doing well. Keep up your healthy habits.", Mild: "Some signs present. Try mindfulness or journaling.", Moderate: "Noticeable impact. Consider speaking to a counsellor.", Severe: "Significant impact. Please reach out to a mental health professional." };
  document.getElementById("result-cards").innerHTML = [
    { label: "Depression", level: depression }, { label: "Anxiety", level: anxiety }, { label: "Stress", level: stress }
  ].map((c, i) =>
    "<div class='result-card fade-in' style='animation-delay:" + (i * 0.15) + "s'><p style='font-weight:500; margin-bottom:8px;'>" + c.label + "</p><span class='badge badge-" + c.level + "'>" + c.level + "</span><p style='font-size:0.82rem; color:var(--muted); margin-top:8px; line-height:1.5;'>" + desc[c.level] + "</p></div>"
  ).join("");
}

function saveScreening() { toast("Result already saved to your history.", "success"); }

function selectMood(btn) {
  document.querySelectorAll(".mood-btn").forEach(b => b.style.borderColor = "var(--border)");
  btn.style.borderColor = "var(--primary)";
  selectedMood = btn.dataset.mood; selectedMoodScore = parseInt(btn.dataset.score);
}
...
document.getElementById("journal-tags-input").addEventListener("keydown", function(e) {
  if (e.key === "Enter" && this.value.trim()) { e.preventDefault(); addTag(this.value.trim()); this.value = ""; }
});

function addTag(tag) { if (!journalTags.includes(tag)) { journalTags.push(tag); renderTags(); } }
function removeTag(tag) { journalTags = journalTags.filter(t => t !== tag); renderTags(); }
function renderTags() {
  document.getElementById("tags-container").innerHTML = journalTags.map(t =>
    "<span style='background:#e8f0fe; color:#1a56db; padding:4px 12px; border-radius:20px; font-size:0.82rem; display:flex; align-items:center; gap:6px;'>" + t + " <span onclick=\"removeTag('" + t + "')\" style='cursor:pointer; font-weight:600;'>x</span></span>"
  ).join("");
}
...
async function saveJournal() {
  ...
  try {
    await api("/api/journal/add", "POST", { mood: selectedMood, moodScore: selectedMoodScore, entryText: text, tags: journalTags, date: new Date().toISOString() });
    toast("Journal entry saved!", "success");
    document.getElementById("journal-text").value = "";
    document.getElementById("char-count").innerText = "0 / 2000";
    journalTags = []; renderTags(); selectedMood = null; selectedMoodScore = null;
    document.querySelectorAll(".mood-btn").forEach(b => b.style.borderColor = "var(--border)");
    loadJournal();
  } catch(e) { toast(e.message, "error"); }
}
...
async function loadJournal() {
  try { allEntries = await api("/api/journal/entries") || []; renderEntries(allEntries); }
  catch(e) { toast("Could not load journal entries", "error"); }
}

function filterEntries() {
  const q = document.getElementById("journal-search").value.toLowerCase();
  renderEntries(allEntries.filter(e => (e.entryText || "").toLowerCase().includes(q) || (e.tags || []).some(t => t.toLowerCase().includes(q))));
}

function renderEntries(entries) {
  const emoji = { "Very Happy": "&#128516;", "Happy": "&#128578;", "Neutral": "&#128528;", "Sad": "&#128532;", "Very Sad": "&#128546;" };
  document.getElementById("entries-list").innerHTML = entries.length
    ? entries.map(e =>
        ...
        "<div style='display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;'><span style='font-size:1.3rem;'>" + (emoji[e.mood] || "&#128528;") + "</span><span style='font-size:0.8rem; color:var(--muted);'>" + (e.date || "").slice(0,10) + "</span></div>" +
        ...
        "<div style='display:flex; flex-wrap:wrap; gap:6px; margin-top:10px;'>" + (e.tags || []).map(t => "<span style='background:#e8f0fe; color:#1a56db; padding:2px 10px; border-radius:20px; font-size:0.78rem;'>" + t + "</span>").join("") + "</div>" +
        "<div style='display:flex; gap:10px; margin-top:10px;'><button onclick=\"event.stopPropagation(); deleteEntry('" + e._id + "')\" style='font-size:0.8rem; color:#9b1c1c; background:none; border:none; cursor:pointer;'>Delete</button></div></div>"
      ).join("")
...
async function deleteEntry(id) {
  ...
  try { await api("/api/journal/" + id, "DELETE"); toast("Entry deleted", "success"); loadJournal(); }
  catch(e) { toast(e.message, "error"); }
}
...
if __name__ == "__main__":
    print(f"MindScan running at http://localhost:{PORT}")
